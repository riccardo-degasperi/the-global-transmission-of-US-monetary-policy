...
%                                                                         %
% Replica of Figure D.1                                                   %
%                                                                         %
% URL to the file:                                                        %
% Fig.D.1 : ./plots/fig_D1/Global_BVAR_multishocks_varplot_AEIP_1.pdf     %
%                                                                         %
...
IP = {'AEIPN';'AEIP';'AEIPU';'AEIPP';'OECDIP'};
CP = {'AECPIN';'AECPI';'AECPIU';'AECPIP';'OECDCPI'};
UU = size(IP,1);
...
for z = 1:UU
    ...
    clearvars -except z UU IP CP stoBVAR stoBVARchannels
    close all
    clc
    ...
    modelOpt.sim            = 2000;                 %number of simulations
    modelOpt.burnin         = 1000;                 %burnin
    ...
    dataOpt.varendo         = {IP{z},CP{z},'OECDSPRDUeom','GCBCFCI','GCBCRA','GCBCFIHUSD','GCBCEHUSD','EMINFLOW','EMOUTFLOW','DIRADV','EURUSD','GBPUSD','JPYUSD','GEAI'};
    ...
    dataOpt.varplot         = {IP{z},CP{z},'OECDSPRDUeom','GCBCFCI','GCBCRA','GCBCFIHUSD','GCBCEHUSD','EMINFLOW','EMOUTFLOW','DIRADV','EURUSD','GBPUSD','JPYUSD','RCRBPI','ROILP','GEAI'};   %selected variables to plot
    ...
    modelOpt.shutVar      = {IP{z},CP{z},'OECDSPRDUeom','GCBCFCI','GCBCRA','GCBCFIHUSD','GCBCEHUSD','EMINFLOW','EMOUTFLOW','DIRADV','EURUSD','GBPUSD','JPYUSD'};   %parameters whose prior variance is 0
    ...
    plotOpt.folder          = ['fig_D1/',IP{z}];    %either a folder name or []
    ...
    sto.ssaOpt   = ssaOpt;
    sto.dims     = dims;
    save([pwd '/plots/',plotOpt.folder,'/output_',IP{z}],'-struct','sto')
...
for z = 1:numel(IP)
stoBVAR_varplot.(IP{z}) = stoBVAR.(IP{z})(iPlot,iPlot);
end
varplot = varendo(iPlot);
longnames = varendolong(iPlot);

longnames(1:2) = {'AEs ex. US & OECD production','AEs ex. US & OECD CPI'};

plotOpt.ylab = ylab0(iPlot);
legNames = {'Dallas','baseline'};

for i = 1:numel(IP)-1

    tmp.(IP{i}) = stoBVAR_varplot.(IP{i});
    tmp.(IP{end}) = stoBVAR_varplot.(IP{end});
    chartName = ['Global_BVAR_multishocks_varplot_',IP{i}];
    IRFdisplay_stack(tmp,varplot,longnames,shockVar,legNames,'varplot',plotOpt,chartName)
    clear tmp
end
...
for z = 1:numel(IP)
stoBVAR_nonvarplot.(IP{z}) = stoBVAR.(IP{z})(iPlot2,iPlot2);
end
...
for i = 1:numel(IP)-1
    ...
    tmp.(IP{i}) = stoBVAR_nonvarplot.(IP{i});
    tmp.(IP{end}) = stoBVAR_nonvarplot.(IP{end});
    chartName = ['Global_BVAR_multishocks_nonvarplot_',IP{i}];
    IRFdisplay_stack(tmp,nonvarplot,longnames,shockVar,legNames,'varplot',plotOpt,chartName)
    clear tmp
end 
