...
for z = 1:UU
    ...
    dataOpt.varendo         = {'IP','CPI','SPRDUeom','XtoM','TVOL','NER','POL','IR','GBY','CBCFCI','CBCRA','CBCXFI'};
    dataOpt.varendo_global  = {'USIP','USCPI','USSPRDUeom','USXtoM','USTVOL','USNER','USGBY','USCBCFCI','USCBCRA','USCBCXFI','GZEBP','VIX','ROILP','GEAI','GS1'};
    ...
    plotOpt.folder          = 'fig_D4/EMs';         %either a folder name or []
    
    plotOpt.allSeries.plotRows = 5;                 %max number of rows per plot
    ...
    stoFolder = 'sto_EMs_MARext';                      %folder where to save IRF mat files
...
% Load country responses
IRF = load([stoFolder,'/stoBVAR_t']);

% Select variables to plot
dataOpt.varplot = {'IP','CPI','SPRDUeom','TVOL','NER','POL','IR','GBY','GS1'};

% Fix dimensions
plotOpt.varplot.plotRows      = 2;
plotOpt.varplot.plotCols      = 4;
plotOpt.varplot.yPlotLength   = 12;
plotOpt.varplot.xPlotLength   = 24;
...
% Generate group 1 IRFs
[IRFulm.open,IRFulm_plot.open] = overlayGroupResponses(IRF,[],[],modelOpt,dataOpt,plotOpt);
...
% Generate group 2 IRFs
[IRFulm.closed,IRFulm_plot.closed] = overlayGroupResponses(IRF,[],[],modelOpt,dataOpt,plotOpt);


%-------------------------------------------------------------------------%
% Overlaid asymmetries plot
folder0 = plotOpt.folder;
plotOpt.folder = [plotOpt.folder,'/openness'];
legNames = {'More Open EMs','Less Open EMs'};   %legend
chartName = 'EM_openness_varendo';
IRFdisplay_asymmetry(IRFulm,dataOpt.varendo,dataOpt.varendolong,dataOpt.shockVar,legNames,'allSeries',plotOpt,chartName)

for i = 1:numel(dataOpt.varplot)
    iPlot0(i,:) = strcmp(dataOpt.varendo,dataOpt.varplot{i});
end
iPlot = sum(iPlot0,1) == 1;
longnames = dataOpt.varendolong(iPlot);

chartName = 'EM_openness_varplot';
IRFdisplay_asymmetry(IRFulm_plot,dataOpt.varplot,longnames,dataOpt.shockVar,legNames,'varplot',plotOpt,chartName)
