function [logML,beta,sigma]=logMLBLP_formin_mod(par,Y,X,p,m,T,N,b,MIN,MAX,s20,l20,l5,iRW,iPrior,Ybar,Xbar,hyperpriors,priorcoef)
...
%
% Modified: 27/07/2015 miranda 2015 smirandaagrippino@london.edu
% Last modified: Riccardo Degasperi 01/07/2019

% Only difference with logMLVAR is that lag dependence is shut down.
...
%-------------------------------------------------------------------------%
% OPTIONS
test = 0;    %estimates BVAR using Banbura, Giannone, Reichlin (2010) to
             %check eventual differences between GLP and BGR codes.
...
%-------------------------------------------------------------------------%
% Setup priors and BVAR estimation

%=========================================================================%
%% Test differences between our BVAR code and GLP's one.
if test
    
    % Generate new original data to avoid overwriting
    Yt = Y;
    Xt = X;
    
    % Retrieve univariate AR(p) residual standard deviations 
    SS = sqrt(s2);

    % Generate dummy observations for NIW prior
    Yrw  = [diag(SS.*iRW)/l1;
            zeros(N*(p-1),N);
            diag(SS)        ;
            zeros(m,N)     ];

    Xrw  = [kron(eye(p),diag(SS)/l1) zeros(N*p,m)           ;
            zeros(N,N*p)             zeros(N,m)             ;
            zeros(m,N*p)             kron(1/(l1*l5),eye(m))];

    % Generate dummies for SoC and Cop (constant and exogenous on bottom)
    ...
    % Estimate BVAR
    %out = VAR(Y,X,Yt,Xt); % This works only for RW Minnesota prior mean.
    
    [TT,NN] = size(Xt);
    
    betah  = (Xt'*Xt + Xrw'*Xrw)\(Xt'*Yt + Xrw'*Xrw*b);
    eh     = Y - X*betah;
    sigmah = (eh'*eh)/(TT-NN);
...
for i = 1:p
    %omega((i-1)*N+1:i*N)=(d-N-1)*((l1/(i^l2))^2)./s2;  %original
    omega((i-1)*N+1:i*N)=(d-N-1)*(l1^2)./s2;            %no lag dependence
end
