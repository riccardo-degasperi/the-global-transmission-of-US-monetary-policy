function [ ] = IRFdisplay_SSA_bands(IRFstruc,varlist,longnames,shockVar,legNames,plotCase,plotOpt,chartName)
...
cbands = [.75 .75 .75 .65 .65 .65;.55 .55 .55 .45 .45 .45];
cmedian = [.2 .4 .6;.2 .2 .2];
...
for t = 1:Nfig
    ...
    for i = (t-1)*rows*cols+1:t*rows*cols
        ...
        %for j = 1:numel(names)
            ...
            % Plot counterfactual bands
            tmp = IRFstruc.(names{2}){i,iShock};
            Xpatch1=[(0:1:h-1) (h-1:-1:0)];
            Ypatch1=[tmp(1,:) fliplr(tmp(5,:))];
            IRFpatch1=patch(Xpatch1,Ypatch1,cbands(2,4:6));  %build confidence bands 90%
            set(IRFpatch1,'facealpha',.5);                   %add transparency
            set(IRFpatch1,'edgecolor','none');               %remove edge color

            % Plot counterfactual median only
            %tmp = repmat(tmp(3,:),5,1);
            tmp0 = [tmp0 tmp];
