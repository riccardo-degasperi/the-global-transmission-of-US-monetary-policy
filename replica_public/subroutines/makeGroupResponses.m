function [IRFulm] = makeGroupResponses(in,makeGroupCase,plotPrefix,modelOpt,dataOpt,plotOpt)
...
for kk = 1:C
    ...
    %---------------------------------------------------------------------%
    % Plot IRFs
    if ~strcmp(makeGroupCase,'asymmetries') %avoid for case asymmetries
    ...
    % if C>1                                  %avoid colour and varplot when plotting channels
    %     whichPlot = 'varendo';
    % end
    ...
    switch whichPlot
        ...
        % Varendo
        chartName = ['AA_',plotPrefix,'_',aggregation,tag,'_varendo'];
        IRFdisplay_single(IRFulm,varendo,varendolong,shockVar,chartName,'allSeries',plotOpt)
        
        % Varplot
        longnames = varendolong(iPlot);
        chartName = ['AA_',plotPrefix,'_',aggregation,tag,'_varplot'];
        IRFdisplay_single(IRFulm_plot,varplot,longnames,shockVar,chartName,'varplot',plotOpt)
        
        % Non varplot
        iPlot2 = ~iPlot;             %policy variable missing
        posShock = find(strcmp(varendo,shockVar));
        iPlot2(posShock) = 1;        %reincluding policy variable
        IRFulm_plot2 = IRFulm(iPlot2,iPlot2);
        longnames = varendolong(iPlot2);
        nonvarplot = varendo(iPlot2);
        chartName = ['AA_',plotPrefix,'_',aggregation,tag,'_nonvarplot'];
        IRFdisplay_single(IRFulm_plot2,nonvarplot,longnames,shockVar,chartName,'varplot',plotOpt)
...
%=============================================%
% Custom plot
vplot = {'IP','CPI','SPRDUeom','GS1'};
iPlot = ismember(dataOpt.varendo,vplot);
rest = fields(chIRFulmStruc);
for j = 1:numel(rest)
    chIRFsulm2.(rest{j}) = chIRFulmStruc.(rest{j})(iPlot,iPlot);
end
chartName = ['AA_',plotPrefix,'_',aggregation,'_overlaid_custom'];
plotOpt.allSeries.plotRows = 1;
plotOpt.allSeries.plotCols = 3;
IRFdisplay_structure(chIRFsulm2,varendo(iPlot),varendolong(iPlot),shockVar,legNames,style,'allSeries',plotOpt,chartName)
%=============================================%
