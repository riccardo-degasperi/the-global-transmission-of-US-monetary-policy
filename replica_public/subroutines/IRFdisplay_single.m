...
cbands = [.75 .75 .75 .65 .65 .65];
cmedian = [.2 .4 .6];

% AE
% cbands = [.55 .55 .55 .45 .45 .45];
% cmedian = [0 .5 .5];
...
for ii = 1:numel(shockVar)
    ...
    for t = 1:Nfig
        ...
        for i = (t-1)*rows*cols+1:t*rows*cols
            ...
            IRFpatch1=patch(Xpatch1,Ypatch1,cbands(1:3));  %build confidence bands 90%
            ...
            IRFpatch2=patch(Xpatch2,Ypatch2,cbands(4:6));  %build confidence bands 68%
            set(IRFpatch2,'facealpha',.5);                   %add transparency
            set(IRFpatch2,'edgecolor','none');               %remove edge color

            % Plot median and X-axis
            plot(0:h-1,tmp(3,:),'Color',cmedian,'LineWidth',2); %plot median
