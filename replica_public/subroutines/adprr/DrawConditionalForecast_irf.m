function [out,flag] = DrawConditionalForecast_irf(bs,Ms,SSArest,iImpulse,dims)
...
if isfield(SSArest,'fixImpulse')     %patch for compatibility with v2.0.3
fixImpulse   = SSArest.fixImpulse;
else
fixImpulse   = 0;
end
...
for s = 1:draws
    ...
    %---------------------------------------------------------------------%
    % If fixPolicy is active, impose equality of IRF of impulse variable
    if fixImpulse
        
        yCondition(:,iImpulse) = irf_bl(:,iImpulse);
